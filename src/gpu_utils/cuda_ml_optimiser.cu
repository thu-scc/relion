...
void getFourierTransformsAndCtfs(
        std::vector<unsigned> &my_ori_particle,
        std::vector<OptimisationParamters> &op,
        std::vector<SamplingParameters> &sp,
        MlOptimiser *baseMLO,
...
#endif
    //FourierTransformer transformer[num];
    CUSTOM_ALLOCATOR_REGION_NAME("GFTCTF");

    std::vector<Image<RFLOAT>> img, rec_img;
    std::vector<MultidimArray<Complex>> Fimg;
    std::vector<MultidimArray<Complex>> Faux;
    std::vector<MultidimArray<RFLOAT>> Fctf;
    std::vector<size_t> indexs, iparts;
    std::vector<RFLOAT> normcorr;
    std::vector<Matrix1D<RFLOAT>> my_old_offset, my_prior;
    std::vector<long int> part_id;
    std::vector<int> group_id, istop;
    std::vector<int> img_size;
    std::vector<CudaGlobalPtr<XFLOAT>> d_img, temp;
    std::vector<int> STBsize;
    std::vector<RFLOAT>rot_deg, tilt_deg, psi_deg;
    std::vector<Matrix1D<RFLOAT>> my_old_offset_helix_coords;
    std::vector<size_t> current_size_x, current_size_y, current_size_z;
    std::vector<CudaGlobalPtr<CUDACOMPLEX>> d_Fimg;

    size_t max_num = 0;
    for (size_t index = 0; index < my_ori_particle.size(); index++)
        for (int ipart = 0; ipart < baseMLO->mydata.ori_particles[my_ori_particle[index]].particles_id.size(); ipart++)
            max_num++;

    if(cudaMLO->transformer.size() < max_num)
    {
        cudaMLO->transformer.reserve(max_num);
        cudaMLO->transformer1.reserve(max_num);
        cudaMLO->transformer2.reserve(max_num);
        for (size_t num = cudaMLO->transformer.size(); num < max_num; num++)
        {
            cudaMLO->transformer.emplace_back();
            cudaMLO->transformer1.emplace_back(cudaStreamPerThread, cudaMLO->devBundle->allocator, baseMLO->mymodel.data_dim);
            cudaMLO->transformer2.emplace_back(cudaStreamPerThread, cudaMLO->devBundle->allocator, baseMLO->mymodel.data_dim);
        }
    }

    img.reserve(max_num);
    rec_img.reserve(max_num);
    Fimg.reserve(max_num);
    Faux.reserve(max_num);
    Fctf.reserve(max_num);
    my_old_offset.reserve(max_num);
    my_prior.reserve(max_num);
    d_img.reserve(max_num);
    temp.reserve(max_num);
    my_old_offset_helix_coords.reserve(max_num);
    d_Fimg.reserve(max_num);

    for (size_t index = 0, num = 0; index < my_ori_particle.size(); index++)
    {
        for (int ipart = 0; ipart < baseMLO->mydata.ori_particles[my_ori_particle[index]].particles_id.size(); ipart++, num++)
        {
            CTIC(cudaMLO->timer,"init");
            FileName fn_img;

            img.emplace_back();
            rec_img.emplace_back();
            Fimg.emplace_back();
            Faux.emplace_back(cudaMLO->transformer[num].fFourier,true);
            Fctf.emplace_back();
            indexs.emplace_back(index);
            iparts.emplace_back(ipart);

            // What is my particle_id?
            part_id.emplace_back(baseMLO->mydata.ori_particles[my_ori_particle[index]].particles_id[ipart]);
            // Which group do I belong?
            group_id.emplace_back(baseMLO->mydata.getGroupId(part_id[num]));

            // Get the right line in the exp_fn_img strings (also exp_fn_recimg and exp_fn_ctfs)
            istop.emplace_back(0);
            for (long int ii = baseMLO->exp_my_first_ori_particle; ii < my_ori_particle[index]; ii++)
                istop[num] += baseMLO->mydata.ori_particles[ii].particles_id.size();
            istop[num] += ipart;

            if (!baseMLO->mydata.getImageNameOnScratch(part_id[num], fn_img))
            {
                std::istringstream split(baseMLO->exp_fn_img);
                for (int i = 0; i <= istop[num]; i++)
                    getline(split, fn_img);
            }
            sp[index].current_img = fn_img;

            // Get the norm_correction
            normcorr.emplace_back(DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_NORM));

            // Get the optimal origin offsets from the previous iteration
            my_old_offset.emplace_back(2);
            my_prior.emplace_back(2);
            XX(my_old_offset[num]) = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_XOFF);
            YY(my_old_offset[num]) = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_YOFF);
            XX(my_prior[num])      = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_XOFF_PRIOR);
            YY(my_prior[num])      = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_YOFF_PRIOR);
            // Uninitialised priors were set to 999.
            if (XX(my_prior[num]) > 998.99 && XX(my_prior[num]) < 999.01)
                XX(my_prior[num]) = 0.;
            if (YY(my_prior[num]) > 998.99 && YY(my_prior[num]) < 999.01)
                YY(my_prior[num]) = 0.;

            if (cudaMLO->dataIs3D)
            {
                my_old_offset[num].resize(3);
                my_prior[num].resize(3);
                ZZ(my_old_offset[num]) = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_ZOFF);
                ZZ(my_prior[num])      = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_ZOFF_PRIOR);
                // Unitialised priors were set to 999.
                if (ZZ(my_prior[num]) > 998.99 && ZZ(my_prior[num]) < 999.01)
                    ZZ(my_prior[num]) = 0.;
            }
            ...
            if (baseMLO->mymodel.orientational_prior_mode != NOPRIOR && !(baseMLO->do_skip_align ||baseMLO-> do_skip_rotate))
            {
                ...
                RFLOAT prior_rot = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_ROT_PRIOR);
                RFLOAT prior_tilt = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_TILT_PRIOR);
                RFLOAT prior_psi = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_PSI_PRIOR);
                RFLOAT prior_psi_flip_ratio =  (baseMLO->mymodel.nr_bodies > 1 ) ? 0. : DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_PSI_PRIOR_FLIP_RATIO);
                ...
                if (prior_rot > 998.99 && prior_rot < 999.01)
                    prior_rot = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_ROT);
                if (prior_tilt > 998.99 && prior_tilt < 999.01)
                    prior_tilt = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_TILT);
                ...
                if (prior_psi > 998.99 && prior_psi < 999.01)
                    prior_psi = DIRECT_A2D_ELEM(baseMLO->exp_metadata,op[index]. metadata_offset + ipart, METADATA_PSI);
                ...
                if (baseMLO->do_helical_refine)
                {
                    baseMLO->sampling.selectOrientationsWithNonZeroPriorProbabilityFor3DHelicalReconstruction(prior_rot, prior_tilt, prior_psi,
                                            ...
                                            op[index].pointer_dir_nonzeroprior, op[index].directions_prior, op[index].pointer_psi_nonzeroprior, op[index].psi_prior,
                ...
                long int nr_orients = baseMLO->sampling.NrDirections(0, &op[index].pointer_dir_nonzeroprior) * baseMLO->sampling.NrPsiSamplings(0, &op[index].pointer_psi_nonzeroprior);
                if (nr_orients == 0)
                {
                    std::cerr << " sampling.NrDirections()= " << baseMLO->sampling.NrDirections(0, &op[index].pointer_dir_nonzeroprior)
                            << " sampling.NrPsiSamplings()= " << baseMLO->sampling.NrPsiSamplings(0, &op[index].pointer_psi_nonzeroprior) << std::endl;
            ...
            if (baseMLO->do_parallel_disc_io)
            {
                ...
                if (baseMLO->do_preread_images)
                {
                    ...
                    img[num]().reshape(baseMLO->mydata.particles[part_id[num]].img);
                    CTIC(cudaMLO->timer,"ParaReadPrereadImages");
                    FOR_ALL_DIRECT_ELEMENTS_IN_MULTIDIMARRAY(baseMLO->mydata.particles[part_id[num]].img)
                    {
                        DIRECT_MULTIDIM_ELEM(img[num](), n) = (RFLOAT)DIRECT_MULTIDIM_ELEM(baseMLO->mydata.particles[part_id[num]].img, n);
                    }
                ...
                if (baseMLO->has_converged && baseMLO->do_use_reconstruct_images)
                {
                    ...
                    rec_img[num].read(fn_recimg);
                    rec_img[num]().setXmippOrigin();
                }
            }
            else
            {
                ...
                if (cudaMLO->dataIs3D)
                {
                    ...
                    img[num]().resize(baseMLO->mymodel.ori_size, baseMLO->mymodel.ori_size,baseMLO-> mymodel.ori_size);
                    ...
                    FOR_ALL_DIRECT_ELEMENTS_IN_ARRAY3D(img[num]())
                    {
                        DIRECT_A3D_ELEM(img[num](), k, i, j) = DIRECT_A3D_ELEM(baseMLO->exp_imagedata, k, i, j);
                    }
                    img[num]().setXmippOrigin();

                    if (baseMLO->has_converged && baseMLO->do_use_reconstruct_images)
                    {
                        rec_img[num]().resize(baseMLO->mymodel.ori_size, baseMLO->mymodel.ori_size,baseMLO-> mymodel.ori_size);
                        int offset = (baseMLO->do_ctf_correction) ? 2 * baseMLO->mymodel.ori_size : baseMLO->mymodel.ori_size;
                        FOR_ALL_DIRECT_ELEMENTS_IN_ARRAY3D(rec_img[num]())
                        {
                            DIRECT_A3D_ELEM(rec_img[num](), k, i, j) = DIRECT_A3D_ELEM(baseMLO->exp_imagedata, offset + k, i, j);
                        }
                        rec_img[num]().setXmippOrigin();
                ...
                else
                {
                    ...
                    img[num]().resize(baseMLO->mymodel.ori_size, baseMLO->mymodel.ori_size);
                    FOR_ALL_DIRECT_ELEMENTS_IN_ARRAY2D(img[num]())
                    {
                        DIRECT_A2D_ELEM(img[num](), i, j) = DIRECT_A3D_ELEM(baseMLO->exp_imagedata, op[index].metadata_offset + ipart, i, j);
                    }
                    img[num]().setXmippOrigin();
                    if (baseMLO->has_converged && baseMLO->do_use_reconstruct_images)
                    {
                        ...
                        rec_img[num]().resize(baseMLO->mymodel.ori_size, baseMLO->mymodel.ori_size);
                        FOR_ALL_DIRECT_ELEMENTS_IN_ARRAY2D(rec_img[num]())
                        {
                            DIRECT_A2D_ELEM(rec_img[num](), i, j) = DIRECT_A3D_ELEM(baseMLO->exp_imagedata, baseMLO->exp_nr_images + op[index].metadata_offset + ipart, i, j);
                        }
                        rec_img[num]().setXmippOrigin();
                    }
...
//            CTIC(cudaMLO->timer,"selfTranslate");
    ...
    for (size_t index = 0, num = 0; index < my_ori_particle.size(); index++)
    {
        for (int ipart = 0; ipart < baseMLO->mydata.ori_particles[my_ori_particle[index]].particles_id.size(); ipart++, num++)
        {
            img_size.emplace_back(img[num].data.nzyxdim);
            d_img.emplace_back((size_t)img_size[num],(cudaStream_t)0,cudaMLO->devBundle->allocator);
            temp.emplace_back((size_t)img_size[num],(cudaStream_t)0,cudaMLO->devBundle->allocator);
            d_img[num].device_alloc();
            temp[num].device_alloc();
            d_img[num].device_init(0);

            for (int i=0; i<img_size[num]; i++)
                temp[num][i] = img[num].data.data[i];

            temp[num].cp_to_device();

            STBsize.emplace_back((int)ceilf((float)img_size[num]/(float)BLOCK_SIZE));
        }
    }
    for(size_t num = 0; num < temp.size(); num++) temp[num].streamSync();
    ...
    for (size_t index = 0, num = 0; index < my_ori_particle.size(); index++)
    {
        for (int ipart = 0; ipart < baseMLO->mydata.ori_particles[my_ori_particle[index]].particles_id.size(); ipart++, num++)
        {
            ...
            if (baseMLO->do_norm_correction)
            {
                ...
                cuda_kernel_multi<<<STBsize[num],BLOCK_SIZE>>>(
                                        ~temp[num],
                                        (XFLOAT)(baseMLO->mymodel.avg_norm_correction / normcorr[num]),
                                        img_size[num]);
    ...
    for (size_t index = 0, num = 0; index < my_ori_particle.size(); index++)
    {
        for (int ipart = 0; ipart < baseMLO->mydata.ori_particles[my_ori_particle[index]].particles_id.size(); ipart++, num++)
        {
            ...
            my_old_offset_helix_coords.emplace_back();
            rot_deg.emplace_back(DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_ROT));
            tilt_deg.emplace_back(DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_TILT));
            psi_deg.emplace_back(DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_PSI));
            if ( (baseMLO->do_helical_refine) && (! baseMLO->ignore_helical_symmetry) )
            {
                // Calculate my_old_offset_helix_coords from my_old_offset[num] and psi angle
                transformCartesianAndHelicalCoords(my_old_offset[num], my_old_offset_helix_coords[num], rot_deg[num], tilt_deg[num], psi_deg[num], CART_TO_HELICAL_COORDS);
                ...
                if ( (baseMLO->mymodel.ref_dim == 3) && (! baseMLO->do_skip_align) && (! baseMLO->do_skip_rotate) )
                {
                    ...
                    {
                        if (! cudaMLO->dataIs3D)
                            XX(my_old_offset_helix_coords[num]) = 0.;
                        else
                            ZZ(my_old_offset_helix_coords[num]) = 0.;
                    }
                }
                // TODO: Now re-calculate the my_old_offset[num] in the real (or image) system of coordinate (rotate -psi angle)
                transformCartesianAndHelicalCoords(my_old_offset_helix_coords[num], my_old_offset[num], rot_deg[num], tilt_deg[num], psi_deg[num], HELICAL_TO_CART_COORDS);
            }
    ...
    for (size_t index = 0, num = 0; index < my_ori_particle.size(); index++)
    {
        for (int ipart = 0; ipart < baseMLO->mydata.ori_particles[my_ori_particle[index]].particles_id.size(); ipart++, num++)
        {
            ...
            if(cudaMLO->dataIs3D)
                cuda_kernel_translate3D<<<STBsize[num],BLOCK_SIZE>>>(
                                    ~temp[num],  // translate from temp[num]...
                                    ~d_img[num], // ... into d_img[num]
                                    img_size[num],
                                    img[num].data.xdim,
                                    img[num].data.ydim,
                                    img[num].data.zdim,
                                    XX(my_old_offset[num]),
                                    YY(my_old_offset[num]),
                                    ZZ(my_old_offset[num]));
            else
                cuda_kernel_translate2D<<<STBsize[num],BLOCK_SIZE>>>(
                                    ~temp[num],  // translate from temp[num]...
                                    ~d_img[num], // ... into d_img[num]
                                    img_size[num],
                                    img[num].data.xdim,
    ...
    for (size_t index = 0, num = 0; index < my_ori_particle.size(); index++)
    {
        for (int ipart = 0; ipart < baseMLO->mydata.ori_particles[my_ori_particle[index]].particles_id.size(); ipart++, num++)
        {
            if (baseMLO->has_converged && baseMLO->do_use_reconstruct_images) //rec_img[num] is NOT norm_corrected in the CPU-code, so nor do we.
            {
                for (int i=0; i<img_size[num]; i++)
                    temp[num][i] = rec_img[num].data.data[i];
                temp[num].cp_to_device();
            }
        }
    }
    ...
    for (size_t index = 0, num = 0; index < my_ori_particle.size(); index++)
    {
        for (int ipart = 0; ipart < baseMLO->mydata.ori_particles[my_ori_particle[index]].particles_id.size(); ipart++, num++)
        {
            if ( (baseMLO->do_helical_refine) && (! baseMLO->ignore_helical_symmetry) )
            {
                ...
                transformCartesianAndHelicalCoords(my_old_offset[num], my_old_offset_helix_coords[num], rot_deg[num], tilt_deg[num], psi_deg[num], CART_TO_HELICAL_COORDS);
                op[index].old_offset[ipart] = my_old_offset_helix_coords[num];
            }
            else
            {
                op[index].old_offset[ipart] = my_old_offset[num];  // Not doing helical refinement. Rounded Cartesian offsets are stored.
            }
            // Also store priors on translations
            op[index].prior[ipart] = my_prior[num];

//            CTOC(cudaMLO->timer,"selfTranslate");
//
//            CTIC(cudaMLO->timer,"calcFimg");
            current_size_x.emplace_back(baseMLO->mymodel.current_size / 2 + 1);
            current_size_y.emplace_back(baseMLO->mymodel.current_size);
            current_size_z.emplace_back((cudaMLO->dataIs3D) ? baseMLO->mymodel.current_size : 1);

            cudaMLO->transformer1[num].setSize(img[num]().xdim,img[num]().ydim,img[num]().zdim);

            //FIXME What is this?
//            deviceInitValue(cudaMLO->transformer1[num].reals, (XFLOAT)0.);
//            deviceInitComplexValue(cudaMLO->transformer1[num].fouriers, (XFLOAT)0.);
//            cudaMLO->transformer1[num].reals.streamSync();
//            cudaMLO->transformer1[num].fouriers.streamSync();

            d_img[num].cp_on_device(cudaMLO->transformer1[num].reals);
        }
    }
    ...
    for (size_t index = 0, num = 0; index < my_ori_particle.size(); index++)
    {
        ...
        {
            runCenterFFT(
                    cudaMLO->transformer1[num].reals,
                    (int)cudaMLO->transformer1[num].xSize,
                    (int)cudaMLO->transformer1[num].ySize,
                    (int)cudaMLO->transformer1[num].zSize,
                    false
                    );
        }
    }
    for(size_t num = 0; num < cudaMLO->transformer1.size(); num++) cudaMLO->transformer1[num].reals.streamSync();

    for (size_t index = 0, num = 0; index < my_ori_particle.size(); index++)
    {
        for (int ipart = 0; ipart < baseMLO->mydata.ori_particles[my_ori_particle[index]].particles_id.size(); ipart++, num++)
        {
            cudaMLO->transformer1[num].forward();
        }
    }
    for(size_t num = 0; num < cudaMLO->transformer1.size(); num++) cudaMLO->transformer1[num].fouriers.streamSync();

    for (size_t index = 0, num = 0; index < my_ori_particle.size(); index++)
    {
        for (int ipart = 0; ipart < baseMLO->mydata.ori_particles[my_ori_particle[index]].particles_id.size(); ipart++, num++)
        {
            int FMultiBsize = ( (int) ceilf(( float)cudaMLO->transformer1[num].fouriers.getSize()*2/(float)BLOCK_SIZE));
            cuda_kernel_multi<<<FMultiBsize,BLOCK_SIZE,0,cudaMLO->transformer1[num].fouriers.getStream()>>>(
                            (XFLOAT*)~cudaMLO->transformer1[num].fouriers,
                            (XFLOAT)1/((XFLOAT)(cudaMLO->transformer1[num].reals.getSize())),
                            cudaMLO->transformer1[num].fouriers.getSize()*2);
            LAUNCH_PRIVATE_ERROR(cudaGetLastError(),cudaMLO->errorStatus);

            d_Fimg.emplace_back(current_size_x[num] * current_size_y[num] * current_size_z[num], cudaMLO->devBundle->allocator);
            d_Fimg[num].device_alloc();
        }
    }
    for(size_t num = 0; num < cudaMLO->transformer1.size(); num++) cudaMLO->transformer1[num].fouriers.streamSync();
    ...
    for (size_t index = 0, num = 0; index < my_ori_particle.size(); index++)
    {
        for (int ipart = 0; ipart < baseMLO->mydata.ori_particles[my_ori_particle[index]].particles_id.size(); ipart++, num++)
        {
            windowFourierTransform2(
                    cudaMLO->transformer1[num].fouriers,
                    d_Fimg[num],
                    cudaMLO->transformer1[num].xFSize,cudaMLO->transformer1[num].yFSize, cudaMLO->transformer1[num].zFSize, //Input dimensions
                    current_size_x[num], current_size_y[num], current_size_z[num]  //Output dimensions
                    );
//            CTOC(cudaMLO->timer,"calcFimg");
            d_Fimg[num].cp_to_host();
        }
    }
    for(size_t num = 0; num < cudaMLO->transformer1.size(); num++) cudaMLO->transformer1[num].fouriers.streamSync();
    ...
    for (size_t index = 0, num = 0; index < my_ori_particle.size(); index++)
    {
        for (int ipart = 0; ipart < baseMLO->mydata.ori_particles[my_ori_particle[index]].particles_id.size(); ipart++, num++)
        {
            ...
            Fimg[num].initZeros(current_size_z[num], current_size_y[num], current_size_x[num]);
            for (int i = 0; i < Fimg[num].nzyxdim; i ++)
            {
                Fimg[num].data[i].real = (RFLOAT) d_Fimg[num][i].x;
                Fimg[num].data[i].imag = (RFLOAT) d_Fimg[num][i].y;
            }
    ...
    for (size_t index = 0, num = 0; index < my_ori_particle.size(); index++)
    {
        for (int ipart = 0; ipart < baseMLO->mydata.ori_particles[my_ori_particle[index]].particles_id.size(); ipart++, num++)
        {
            ...
            //
            RFLOAT beamtilt_x = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_BEAMTILT_X);
            RFLOAT beamtilt_y = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_BEAMTILT_Y);
            RFLOAT Cs = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_CTF_CS);
            RFLOAT V = 1000. * DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_CTF_VOLTAGE);
            RFLOAT lambda = 12.2643247 / sqrt(V * (1. + V * 0.978466e-6));
            if (ABS(beamtilt_x) > 0. || ABS(beamtilt_y) > 0.)
                selfApplyBeamTilt(Fimg[num], beamtilt_x, beamtilt_y, lambda, Cs,baseMLO->mymodel.pixel_size, baseMLO->mymodel.ori_size);

            op[index].Fimgs_nomask.at(ipart) = Fimg[num];
            ...
            if (!baseMLO->do_zero_mask)
            {
                ...
                // Have the seed based on the part_id[num], so that each particle has a different instant of the noise
                if (baseMLO->do_realign_movies)
                    init_random_generator(baseMLO->random_seed + part_id[num]);
                else
                    init_random_generator(baseMLO->random_seed + my_ori_particle[index]); // This only serves for exact reproducibility tests with 1.3-code...
                ...
                MultidimArray<RFLOAT> power_noise = baseMLO->sigma2_fudge * baseMLO->mymodel.sigma2_noise[group_id[num]];
                if (baseMLO->do_realign_movies)
                    power_noise *= (2. * baseMLO->movie_frame_running_avg_side + 1.);

                // Create noisy image for outside the mask
                MultidimArray<Complex > Fnoise;
                Mnoise.resize(img[num]());
                cudaMLO->transformer[num].setReal(Mnoise);
                cudaMLO->transformer[num].getFourierAlias(Fnoise);
                ...
                cudaMLO->transformer[num].inverseFourierTransform();
                ...
                d_img[num].cp_to_host();
                d_img[num].streamSync();
                for (int i=0; i<img_size[num]; i++)
                    img[num].data.data[i] = d_img[num][i];

                if (is_helical_segment)
                {
                    softMaskOutsideMapForHelix(img[num](), psi_deg[num], tilt_deg[num], (baseMLO->particle_diameter / (2. * baseMLO->mymodel.pixel_size)),
                            (baseMLO->helical_tube_outer_diameter / (2. * baseMLO->mymodel.pixel_size)), baseMLO->width_mask_edge, &Mnoise);
                }
                else
                    softMaskOutsideMap(img[num](), baseMLO->particle_diameter / (2. * baseMLO->mymodel.pixel_size), (RFLOAT)baseMLO->width_mask_edge, &Mnoise);

                for (int i=0; i<img_size[num]; i++)
                    d_img[num][i] = img[num].data.data[i];
                d_img[num].cp_to_device();
            ...
            else
            {
                ...
                if (radius < 0)
                    radius = ((RFLOAT)img[num].data.xdim)/2.;
                XFLOAT radius_p = radius + cosine_width;

//                dim3 block_dim = 1; //TODO
//                cuda_kernel_softMaskOutsideMap<<<block_dim,SOFTMASK_BLOCK_SIZE>>>(    ~d_img[num],
//                                                                                    img[num]().nzyxdim,
//                                                                                    img[num].data.xdim,
//                                                                                    img[num].data.ydim,
//                                                                                    img[num].data.zdim,
...
//                LAUNCH_PRIVATE_ERROR(cudaGetLastError(),cudaMLO->errorStatus);
                ...
                cuda_kernel_softMaskBackgroundValue<<<block_dim,SOFTMASK_BLOCK_SIZE>>>(    ~d_img[num],
                                                                                    img[num]().nzyxdim,
                                                                                    img[num].data.xdim,
                                                                                    img[num].data.ydim,
                                                                                    img[num].data.zdim,
                ...
                cuda_kernel_cosineFilter<<<block_dim,SOFTMASK_BLOCK_SIZE>>>(    ~d_img[num],
                                                                                img[num]().nzyxdim,
                                                                                img[num].data.xdim,
                                                                                img[num].data.ydim,
                                                                                img[num].data.zdim,
...
//                FOR_ALL_DIRECT_ELEMENTS_IN_MULTIDIMARRAY(img[num]())
//                {
//                    img[num].data.data[n]=(RFLOAT)d_img[num][n];
//                }
...
//            CTIC(cudaMLO->timer,"transform");
    ...
    for (size_t index = 0, num = 0; index < my_ori_particle.size(); index++)
    {
        for (int ipart = 0; ipart < baseMLO->mydata.ori_particles[my_ori_particle[index]].particles_id.size(); ipart++, num++)
        {
            int FMultiBsize2 = ( (int) ceilf(( float)cudaMLO->transformer1[num].fouriers.getSize()*2/(float)BLOCK_SIZE));
            cuda_kernel_multi<<<FMultiBsize2,BLOCK_SIZE,0,cudaMLO->transformer1[num].fouriers.getStream()>>>(
                            (XFLOAT*)~cudaMLO->transformer1[num].fouriers,
                            (XFLOAT)1/((XFLOAT)(cudaMLO->transformer1[num].reals.getSize())),
                            cudaMLO->transformer1[num].fouriers.getSize()*2);
            LAUNCH_PRIVATE_ERROR(cudaGetLastError(),cudaMLO->errorStatus);
...
//            CTOC(cudaMLO->timer,"transform");
            ...
            if (baseMLO->mymodel.current_size < baseMLO->mymodel.ori_size)
            {
                ...
                dim3 gridSize = CEIL((float)(cudaMLO->transformer1[num].fouriers.getSize()) / (float)POWERCLASS_BLOCK_SIZE);
                if(cudaMLO->dataIs3D)
                    cuda_kernel_powerClass<true><<<gridSize,POWERCLASS_BLOCK_SIZE,0,0>>>(
                        ~cudaMLO->transformer1[num].fouriers,
                        ~spectrumAndXi2,
                        cudaMLO->transformer1[num].fouriers.getSize(),
                        spectrumAndXi2.getSize()-1,
                        cudaMLO->transformer1[num].xFSize,
                        cudaMLO->transformer1[num].yFSize,
                        cudaMLO->transformer1[num].zFSize,
                ...
                op[index].power_imgs.at(ipart).resize(baseMLO->mymodel.ori_size/2 + 1);

                for (int i = 0; i<(spectrumAndXi2.getSize()-1); i ++)
                    op[index].power_imgs.at(ipart).data[i] = spectrumAndXi2[i];
                op[index].highres_Xi2_imgs.at(ipart) = spectrumAndXi2[spectrumAndXi2.getSize()-1];
            }
            else
            {
                op[index].highres_Xi2_imgs.at(ipart) = 0.;
            }
    ...
    for (size_t index = 0, num = 0; index < my_ori_particle.size(); index++)
    {
        for (int ipart = 0; ipart < baseMLO->mydata.ori_particles[my_ori_particle[index]].particles_id.size(); ipart++, num++)
        {
            ...
            //windowFourierTransform(Faux[num], Fimg[num], baseMLO->mymodel.current_size);
            windowFourierTransform2(
                    cudaMLO->transformer1[num].fouriers,
                    d_Fimg[num],
                    cudaMLO->transformer1[num].xFSize,cudaMLO->transformer1[num].yFSize, cudaMLO->transformer1[num].zFSize, //Input dimensions
                    ...
                    1,     //Npsi
                    0,    //pos
                    cudaMLO->transformer1[num].fouriers.getStream()
                    );
            CTOC(cudaMLO->timer,"windowFourierTransform2");
        }
    }
    for(size_t num = 0; num < d_Fimg.size(); num++) d_Fimg[num].streamSync();
...
//            CTOC(cudaMLO->timer,"cpFimg2Host_2");
    ...
    for (size_t index = 0, num = 0; index < my_ori_particle.size(); index++)
    {
        for (int ipart = 0; ipart < baseMLO->mydata.ori_particles[my_ori_particle[index]].particles_id.size(); ipart++, num++)
        {
            ...
            if (baseMLO->do_ctf_correction)
            {
                if (cudaMLO->dataIs3D)
                {
                    ...
                    if (baseMLO->do_parallel_disc_io)
                    {
                        ...
                        FileName fn_ctf;
                        if (!baseMLO->mydata.getImageNameOnScratch(part_id[num], fn_ctf, true))
                        {
                    ...
                    // Set the CTF-image in Fctf[num]
                    CTIC(cudaMLO->timer,"CTFSet3D_array");
                    Ictf().setXmippOrigin();
                    FOR_ALL_ELEMENTS_IN_FFTW_TRANSFORM(Fctf[num])
                    {
                        // Use negative kp,ip and jp indices, because the origin in the ctf_img lies half a pixel to the right of the actual center....
                        DIRECT_A3D_ELEM(Fctf[num], k, i, j) = A3D_ELEM(Ictf(), -kp, -ip, -jp);
                    }
                    CTIC(cudaMLO->timer,"CTFSet3D_array");
                }
                else
                {
                    ...
                    CTF ctf;
                    ctf.setValues(DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_CTF_DEFOCUS_U),
                                  DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_CTF_DEFOCUS_V),
                                  DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_CTF_DEFOCUS_ANGLE),
                                  DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_CTF_VOLTAGE),
                                  DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_CTF_CS),
                                  DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_CTF_Q0),
                                  DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_CTF_BFAC),
                                                              1.,
                                                              DIRECT_A2D_ELEM(baseMLO->exp_metadata, op[index].metadata_offset + ipart, METADATA_CTF_PHASE_SHIFT));

                    ctf.getFftwImage(Fctf[num], baseMLO->mymodel.ori_size, baseMLO->mymodel.ori_size, baseMLO->mymodel.pixel_size,
            ...
            {
                Fctf[num].initConstant(1.);
            }
//            CTOC(cudaMLO->timer,"ctfCorr");
            // Store Fimg[num] and Fctf[num]
            op[index].Fimgs.at(ipart) = Fimg[num];
            op[index].Fctfs.at(ipart) = Fctf[num];

        } // end loop ipart
    } // end loop index
    //cudaMLO->transformer[num].clear();
#ifdef TIMING
    //if (op[index].my_ori_particle[index] == baseMLO->exp_my_first_ori_particle)
...
void MlOptimiserCuda::resetData()
{
    ...
    for(size_t num = 0; num < transformer1.size(); num++)
    {
        transformer[num].clear();
        transformer1[num].clear();
        transformer2[num].clear();
    }
    transformer.clear();
    transformer1.clear();
    transformer2.clear();
...
//    CTOC(cudaMLO->timer,"interParticle");
    ...
    while (baseMLO->exp_ipart_ThreadTaskDistributor->getTasks(first_ipart, last_ipart))
    {
        ...
        sp.reserve(last_ipart-first_ipart+1);
        op.reserve(last_ipart-first_ipart+1);
//        std::cout << first_ipart << " " << last_ipart << " v5" << std::endl;
...
//            CTIC(timer,"oneParticle");
    ...
    if (thread_id == 0)
        baseMLO->timer.tic(baseMLO->TIMING_ESP_DIFF2_A);
#endif
            my_ori_particle.emplace_back(baseMLO->exp_my_first_ori_particle + ipart);
            sp.emplace_back();
            sp[ipart-first_ipart].nr_particles = baseMLO->mydata.ori_particles[my_ori_particle[ipart-first_ipart]].particles_id.size();

            op.emplace_back(sp[ipart-first_ipart].nr_particles, my_ori_particle[ipart-first_ipart]);
            ...
            sp[ipart-first_ipart].iclass_min = 0;
            sp[ipart-first_ipart].iclass_max = baseMLO->mymodel.nr_classes - 1;
            ...
            if (baseMLO->do_generate_seeds)
            {
                if (baseMLO->do_firstiter_cc && baseMLO->iter == 1)
                {
                    ...
                    sp[ipart-first_ipart].iclass_min = sp[ipart-first_ipart].iclass_max = 0;
                }
                        ...
                        (!baseMLO->do_firstiter_cc && baseMLO->iter == 1))
                {
                    ...
                    long int idx = my_ori_particle[ipart-first_ipart] - baseMLO->exp_my_first_ori_particle;
                    ...
                    sp[ipart-first_ipart].iclass_min = sp[ipart-first_ipart].iclass_max = baseMLO->exp_random_class_some_particles[idx];
                }
            }

            // Global exp_metadata array has metadata of all ori_particles. Where does my_ori_particle[ipart-first_ipart] start?
            for (long int iori = baseMLO->exp_my_first_ori_particle; iori <= baseMLO->exp_my_last_ori_particle; iori++)
            {
                if (iori == my_ori_particle[ipart-first_ipart]) break;
                op[ipart-first_ipart].metadata_offset += baseMLO->mydata.ori_particles[iori].particles_id.size();
            }
#ifdef TIMING
    // Only time one thread
    if (thread_id == 0)
        ...
        for (long unsigned ipart = first_ipart; ipart <= last_ipart; ipart++)
        {
            ...
            {
                baseMLO->calculateRunningAveragesOfMovieFrames(my_ori_particle[ipart-first_ipart], op[ipart-first_ipart].Fimgs, op[ipart-first_ipart].power_imgs, op[ipart-first_ipart].highres_Xi2_imgs);
            }

            // To deal with skipped alignments/rotations
            if (baseMLO->do_skip_align)
            {
                sp[ipart-first_ipart].itrans_min = sp[ipart-first_ipart].itrans_max = sp[ipart-first_ipart].idir_min = sp[ipart-first_ipart].idir_max = sp[ipart-first_ipart].ipsi_min = sp[ipart-first_ipart].ipsi_max =
                        my_ori_particle[ipart-first_ipart] - baseMLO->exp_my_first_ori_particle;
            }
            else
            {
                sp[ipart-first_ipart].itrans_min = 0;
                sp[ipart-first_ipart].itrans_max = baseMLO->sampling.NrTranslationalSamplings() - 1;

                if (baseMLO->do_skip_rotate)
                {
                    sp[ipart-first_ipart].idir_min = sp[ipart-first_ipart].idir_max = sp[ipart-first_ipart].ipsi_min = sp[ipart-first_ipart].ipsi_max =
                            my_ori_particle[ipart-first_ipart] - baseMLO->exp_my_first_ori_particle;
                }
                else
                {
                    sp[ipart-first_ipart].idir_min = sp[ipart-first_ipart].ipsi_min = 0;
                    sp[ipart-first_ipart].idir_max = baseMLO->sampling.NrDirections(0, &op[ipart-first_ipart].pointer_dir_nonzeroprior) - 1;
                    sp[ipart-first_ipart].ipsi_max = baseMLO->sampling.NrPsiSamplings(0, &op[ipart-first_ipart].pointer_psi_nonzeroprior ) - 1;
                }
            }

            // Initialise significant weight to minus one, so that all coarse sampling points will be handled in the first pass
            op[ipart-first_ipart].significant_weight.resize(sp[ipart-first_ipart].nr_particles, -1.);
            ...
            std::vector <IndexedDataArray> CoarsePassWeights(1, devBundle->allocator) ,FinePassWeights(sp[ipart-first_ipart].nr_particles, devBundle->allocator);
            ...
            std::vector < std::vector <IndexedDataArrayMask> > FinePassClassMasks(sp[ipart-first_ipart].nr_particles, std::vector <IndexedDataArrayMask>(baseMLO->mymodel.nr_classes, devBundle->allocator));
            ...
            std::vector < ProjectionParams > FineProjectionData(sp[ipart-first_ipart].nr_particles, baseMLO->mymodel.nr_classes);

            std::vector < cudaStager<unsigned long> > stagerD2(sp[ipart-first_ipart].nr_particles,devBundle->allocator), stagerSWS(sp[ipart-first_ipart].nr_particles,devBundle->allocator);
    ...
    if (thread_id == 0)
        baseMLO->timer.tic(baseMLO->TIMING_ESP_DIFF2_B);
                ...
                if (baseMLO->strict_highres_exp > 0.)
                    ...
                    sp[ipart-first_ipart].current_image_size = baseMLO->coarse_size;
                else if (baseMLO->adaptive_oversampling > 0)
                    // Use smaller images in the first pass, larger ones in the second pass
                    sp[ipart-first_ipart].current_image_size = (ipass == 0) ? baseMLO->coarse_size : baseMLO->mymodel.current_size;
                else
                    sp[ipart-first_ipart].current_image_size = baseMLO->mymodel.current_size;

                // Use coarse sampling in the first pass, oversampled one the second pass
                sp[ipart-first_ipart].current_oversampling = (ipass == 0) ? 0 : baseMLO->adaptive_oversampling;

                sp[ipart-first_ipart].nr_dir = (baseMLO->do_skip_align || baseMLO->do_skip_rotate) ? 1 : baseMLO->sampling.NrDirections(0, &op[ipart-first_ipart].pointer_dir_nonzeroprior);
                sp[ipart-first_ipart].nr_psi = (baseMLO->do_skip_align || baseMLO->do_skip_rotate) ? 1 : baseMLO->sampling.NrPsiSamplings(0, &op[ipart-first_ipart].pointer_psi_nonzeroprior);
                sp[ipart-first_ipart].nr_trans = (baseMLO->do_skip_align) ? 1 : baseMLO->sampling.NrTranslationalSamplings();
                sp[ipart-first_ipart].nr_oversampled_rot = baseMLO->sampling.oversamplingFactorOrientations(sp[ipart-first_ipart].current_oversampling);
                sp[ipart-first_ipart].nr_oversampled_trans = baseMLO->sampling.oversamplingFactorTranslations(sp[ipart-first_ipart].current_oversampling);
#ifdef TIMING
    // Only time one thread
    if (thread_id == 0)
        baseMLO->timer.toc(baseMLO->TIMING_ESP_DIFF2_B);
                ...
                op[ipart-first_ipart].min_diff2.resize(sp[ipart-first_ipart].nr_particles, 0);
                op[ipart-first_ipart].avg_diff2.resize(sp[ipart-first_ipart].nr_particles, 0);

                if (ipass == 0)
                {
                    unsigned long weightsPerPart(baseMLO->mymodel.nr_classes * sp[ipart-first_ipart].nr_dir * sp[ipart-first_ipart].nr_psi * sp[ipart-first_ipart].nr_trans * sp[ipart-first_ipart].nr_oversampled_rot * sp[ipart-first_ipart].nr_oversampled_trans);

                    op[ipart-first_ipart].Mweight.resizeNoCp(1,1,sp[ipart-first_ipart].nr_particles, weightsPerPart);

                    CudaGlobalPtr<XFLOAT> Mweight(devBundle->allocator);
                    Mweight.setSize(sp[ipart-first_ipart].nr_particles * weightsPerPart);
                    Mweight.setHstPtr(op[ipart-first_ipart].Mweight.data);
                    Mweight.device_alloc();
                    deviceInitValue<XFLOAT>(Mweight, -999.);
                    Mweight.streamSync();

                    CTIC(timer,"getAllSquaredDifferencesCoarse");
                    getAllSquaredDifferencesCoarse(ipass, op[ipart-first_ipart], sp[ipart-first_ipart], baseMLO, this, Mweight);
                    CTOC(timer,"getAllSquaredDifferencesCoarse");

                    try
                    {
                        CTIC(timer,"convertAllSquaredDifferencesToWeightsCoarse");
                        convertAllSquaredDifferencesToWeights<XFLOAT>(ipass, op[ipart-first_ipart], sp[ipart-first_ipart], baseMLO, this, CoarsePassWeights, FinePassClassMasks, Mweight);
                    ...
                    catch (RelionError XE)
                    {
                        ...
                        try {
                            convertAllSquaredDifferencesToWeights<double>(ipass, op[ipart-first_ipart], sp[ipart-first_ipart], baseMLO, this, CoarsePassWeights, FinePassClassMasks, Mweight);
                        }
                        catch (RelionError XE)
#endif
                        {
                            ...
                            convertAllSquaredDifferencesToWeights<XFLOAT>(ipass, op[ipart-first_ipart], sp[ipart-first_ipart], baseMLO, this, CoarsePassWeights, FinePassClassMasks, Mweight, true);
...
//                    // weight-array, which would be insanely much larger than necessary if we had to assume the worst.
                    for (long int iframe = 0; iframe < sp[ipart-first_ipart].nr_particles; iframe++)
                    {
                        FineProjectionData[iframe].orientationNumAllClasses = 0;
                        for (int exp_iclass = sp[ipart-first_ipart].iclass_min; exp_iclass <= sp[ipart-first_ipart].iclass_max; exp_iclass++)
                        {
                            if(exp_iclass>0)
                            ...
                            FineProjectionData[iframe].orientationNumAllClasses += generateProjectionSetupFine(
                                    op[ipart-first_ipart],
                                    sp[ipart-first_ipart],
                                    baseMLO,
                                    exp_iclass,
                        ...
                        FinePassWeights[iframe].setDataSize(FineProjectionData[iframe].orientationNumAllClasses*sp[ipart-first_ipart].nr_trans*sp[ipart-first_ipart].nr_oversampled_trans);
                        FinePassWeights[iframe].dual_alloc_all();
                        stagerD2[iframe].size= 2*(FineProjectionData[iframe].orientationNumAllClasses*sp[ipart-first_ipart].nr_trans*sp[ipart-first_ipart].nr_oversampled_trans);
...
//                    printf("Allocator used space before 'getAllSquaredDifferencesFine': %.2f MiB\n", (float)devBundle->allocator->getTotalUsedSpace()/(1024.*1024.));
                    ...
                    getAllSquaredDifferencesFine(ipass, op[ipart-first_ipart], sp[ipart-first_ipart], baseMLO, this, FinePassWeights, FinePassClassMasks, FineProjectionData, stagerD2);
                    ...
                    convertAllSquaredDifferencesToWeights<XFLOAT>(ipass, op[ipart-first_ipart], sp[ipart-first_ipart], baseMLO, this, FinePassWeights, FinePassClassMasks, Mweight);
    ...
    if (thread_id == 0)
        baseMLO->timer.toc(baseMLO->TIMING_ESP_DIFF2_E);
            ...
            storeWeightedSums(op[ipart-first_ipart], sp[ipart-first_ipart], baseMLO, this, FinePassWeights, FineProjectionData, FinePassClassMasks, stagerSWS);
